...
const StudentLoginForm: FC<StudentLoginFormProps> = ({ onSuccess, onFailure }) => {
  ...
  const submit = async () => {
    try {
      const response = await request.get(`students/email/${encodeURIComponent(email)}`, {
        validateStatus: () => true,
      });
  ...
  return (
    <Flex direction="column" gap={20}>
      <TextInput maw={400} value={email} onChange={update(setEmail)} label="E-mail" />

      <Button maw={240} color="blue" radius="md" onClick={jsSubmit(submit)}>
        Login
      </Button>
    </Flex>
  );
