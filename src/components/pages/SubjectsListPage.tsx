...
const SubjectsListPage: FC = () => {
  ...
  useEffect(() => { // TODO: remove this mock after real request is ready
    setSubjects([{
      subjectId: 234,
  ...
  return <>
    <h1>Subjects list</h1>
    <p>List contains data about all subjects - name and lecturer who directs it.</p>
  <table>
    <thead>
      <tr>
        <td>Subject name</td>
    ...
    <tbody>
    { subjects.map(subject => <tr key={subject.subjectId}><td>{ subject.name }</td><td>{ subject.lecturer.name }</td></tr>) }
    </tbody>
  </table>
  </>
